...
public class BristlyHorseModel<T extends BristlyHorseEntity> extends HierarchicalModel<T> {
    public static final ModelLayerLocation LAYER_LOCATION =
            new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(ExplosiveHorses.MOD_ID, "bristly_horse"), "main");
    ...
    public BristlyHorseModel(ModelPart root) {
    ...
    @Override
    public void setupAnim(BristlyHorseEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);

        this.animateWalk(BristlyHorseAnimations.horsey_walking, limbSwing, limbSwingAmount, 1.2f, 2.5f);
    }
